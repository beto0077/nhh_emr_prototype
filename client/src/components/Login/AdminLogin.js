...
function AdminLogin() {
  const { loginAdmin } = useAdminContext(); // Access the loginDoctor function from DoctorContext
  ...
  const onSubmit = async e => {
    ...
    try {
      const token = await loginAdmin(user); // Call the loginDoctor function
      if(token.status === 401) {
        ...
        <div className="row">
          <div className="col-md-6 mt-5 mx-auto">
            <form noValidate onSubmit={onSubmit}>
              <h1 className="h3 mb-3 mt-5 font-weight-normal btn-rg" style={{marginTop: '100px'}}>
