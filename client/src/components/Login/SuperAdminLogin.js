...
function SuperAdminLogin() {
  const { loginSuperAdmin } = useSuperAdminContext(); // Access the loginDoctor function from DoctorContext
  ...
  const onSubmit = async e => {
    ...
    try {
      const token = await loginSuperAdmin(user); // Call the loginDoctor function
      if(token.status === 401) {
